...
if [ "$1" = "start" ]; then
	...
	[ -f ${LOG_DIR}/host_port.log ] && cat /dev/null > ${LOG_DIR}/host_port.log
	echo "Starting $0 ..."
	exec 1>$LOG_FILE
	exec 2>&1
	nohup "$0" &
...
if [ "$1" = "stop" ]; then
	shift
	[ -f /var/run/dnsbl-ipset.pid ] && kill $(cat /var/run/dnsbl-ipset.pid) $(pgrep -P $(cat /var/run/dnsbl-ipset.pid)|xargs)
	exit $?
fi
...
BLACKLIST_IPSET_OPTIONS="EX $[7 * 24 * 3600]"
...
CACHE_IPSET_OPTIONS="EX $[24 * 3600]"
...
# Redis settings (dario - neomediatech - 07 dec 2021)
REDIS_PORT="6380"
redis_cmd="redis-cli -p $REDIS_PORT --raw"
...
flush	will empty ${BLACKLIST_IPSET} and ${CACHE_IPSET} Redis db
...
do
	case "${1}" in
		...
		flush|clear|clean|restart)
			...
			$redis_cmd -n "${CACHE_IPSET}" 'FLUSHDB'
			#ipset --flush "${BLACKLIST_IPSET}"
			$redis_cmd -n "${BLACKLIST_IPSET}" 'FLUSHDB'
			echo >&2 "Redis DB ${CACHE_IPSET} and ${BLACKLIST_IPSET} have been flushed."
			exit 0
			;;
...
$redis_cmd 'PING' 1>/dev/null 2>/dev/null
#ipset --list "${BLACKLIST_IPSET}" >/dev/null
if [ $? -ne 0 ]
then
	echo >&2 "Cannot connect to Redis service on port $REDIS_PORT"
	echo >&2 "Start Redis and retry"
...
blacklist() {
	...
	if [ ${tscore} -ge ${BLACKLIST_SCORE} ]
		...
		[ -n "$($redis_cmd -n ${BLACKLIST_IPSET} GET ${ip})" ] && return 1
...
generate_dnsbl_hostnames() {
	if [[ "${1}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]
		...
		do
			...
			[ -n "$($redis_cmd -n ${x} GET "${1}")" ] && return 1
		done
...
log_ip_ports() {
		...
		do
			...
			#[ $? -eq 0 ] && found=1
			[ -n "$($redis_cmd -n ${x} GET "${src}")" ] && found=1
		done
...
tail -s 0.2 -F "${IPTABLES_LOG}" |\
	grep --line-buffered -E "^.*${ULOG_MATCH}.* SRC=[0-9.]+ DST=[0-9.]+ .*$" |\
        sed --unbuffered "s/^.* SRC=\([0-9\.]\+\) DST=\([0-9\.]\+\) .*PROTO=\([A-Z]\+\) SPT=[0-9\.]\+ DPT=\([0-9\.]\+\) .*$/\1 \2 \3 \4/g" |\
        log_ip_ports |\
	generate_hostnames_from_src_dst |\
